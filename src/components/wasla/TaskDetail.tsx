...
import { memberById, listById, taskById, type Task } from "@/lib/mock-data";
import { Avatar } from "./Avatar";
import { StatusPill } from "./StatusPill";
import { PriorityIcon } from "./PriorityIcon";
import { FileText, Hash, Paperclip, MessageSquare, CheckSquare, Download, ExternalLink, Plus, MoreHorizontal, Link2, X } from "lucide-react";
    ...
    <div className="grid gap-6 px-6 py-5 lg:grid-cols-3">
      <div className="space-y-6 lg:col-span-2">
        <div className="flex items-start gap-2">
          {editingTitle ? (
            <Input
              ...
              onBlur={() => { if (titleDraft.trim()) updateTask(task.id, { title: titleDraft.trim() }); setEditingTitle(false); }}
              onKeyDown={(e) => {
                if (e.key === "Enter") { updateTask(task.id, { title: titleDraft.trim() }); setEditingTitle(false); }
            ...
            <h1
              ...
              className="flex-1 cursor-text text-2xl font-semibold leading-tight tracking-tight hover:bg-muted/30 rounded px-1 -mx-1"
            >
              {task.title}
            </h1>
          )}
        ...
        <section>
          ...
          <p className="text-sm leading-relaxed text-foreground/85">{task.description || <span className="italic text-muted-foreground/70">No description.</span>}</p>
        </section>
      ...
      <aside className="space-y-4 text-sm lg:sticky lg:top-4 lg:self-start">
        ...
        <Meta label="Watchers">
          <div className="flex -space-x-1.5">
            ...
            {!task.watchers?.length && <span className="text-xs text-muted-foreground">—</span>}
          </div>
        </Meta>
        <Meta label="Tags">
        ...
        <div>
          ...
          <DepGroup label="Blocks" ids={task.dependencies?.blocks ?? []} onRemove={(id) => removeDependency(task.id, id)} onOpen={(id) => goTask(id)} />
          <DepGroup label="Blocked by" ids={task.dependencies?.blockedBy ?? []} onRemove={(id) => removeDependency(id, task.id)} onOpen={(id) => goTask(id)} />
          <Popover>
