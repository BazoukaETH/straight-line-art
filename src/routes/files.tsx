...
import { FileText, FileImage, FileSpreadsheet, Presentation, Search, Plus, Pencil, Trash2, ExternalLink, ChevronDown, ChevronUp } from "lucide-react";
...
function FilesPage() {
  const [tab, setTab] = useState<"drive" | "documents">("drive");
  return (
    <AppShell sidebar={<SpaceTreeSidebar />} breadcrumb={<><span>Files</span><span className="text-border">/</span><span className="font-medium text-foreground">{tab === "drive" ? "Drive" : "Documents"}</span></>}>
      <div className="px-6 py-5">
        <div className="mb-5 flex items-center justify-between">
          <div>
            <h1 className="text-xl font-semibold">Files</h1>
            <p className="text-sm text-muted-foreground">Drive sync and the document directory</p>
          </div>
        ...
        {tab === "drive" ? <DrivePanel /> : <DocumentsPanel />}
      </div>
    </AppShell>
  );
}
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <p className="text-xs text-muted-foreground">Categorized document directory with Google Drive links</p>
