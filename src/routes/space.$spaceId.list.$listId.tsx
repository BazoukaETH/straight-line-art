...
export const Route = createFileRoute("/space/$spaceId/list/$listId")({ component: ListPage });
      ...
      <div className="px-6 py-5">
        <div className="mb-4 flex items-center justify-between gap-3">
          <div className="min-w-0">
            ...
            <p className="text-sm text-muted-foreground">{scopedTasks.length} tasks</p>
          </div>
          <div className="flex items-center gap-2">
        ...
        <Tabs value={tab} onValueChange={(v) => setTab(v as any)}>
          ...
          <TabsContent value="table" className="mt-4"><TableView tasks={scopedTasks} customFields={list?.customFields ?? []} /></TabsContent>
          <TabsContent value="timeline" className="mt-4"><TimelineView tasks={scopedTasks} /></TabsContent>
        </Tabs>
      </div>
      {list && <ListSettingsDialog listId={list.id} open={settingsOpen} onOpenChange={setSettingsOpen} />}
      {/* avoid unused */}
  ...
  return (
    <div className="flex gap-3 overflow-x-auto pb-2">
      {STATUSES.map((col) => {
        ...
        return (
          <div key={col} className="flex w-72 shrink-0 flex-col gap-2 rounded-lg bg-muted/40 p-3" onDragOver={(e) => e.preventDefault()} onDrop={(e) => onDrop(e, col)}>
        ...
        return (
          <div key={i} className="min-h-[110px] bg-card p-1.5">
            ...
            {dayTasks.slice(0, 3).map((t) => (
              <button key={t.id} onClick={() => goTask(t)} className="mb-1 block w-full truncate rounded bg-[color-mix(in_oklab,var(--accent)_14%,transparent)] px-1.5 py-0.5 text-left text-[11px] text-accent hover:bg-[color-mix(in_oklab,var(--accent)_22%,transparent)]">
                {t.title}
              </button>
            ))}
    ...
    <div className="overflow-auto rounded-lg border border-border bg-card">
      <table className="w-full text-sm">
        <thead className="border-b border-border bg-muted/40">
          <tr>
            {head("title", "Title")}{head("status", "Status")}{head("assigneeId", "Assignee")}{head("priority", "Priority")}{head("due", "Due")}{head("createdAt", "Created")}
        ...
        <tbody>
          {sorted.map((t) => {
            ...
            return (
              <tr key={t.id} onClick={() => goTask(t)} className="cursor-pointer border-b border-border/60 hover:bg-muted/40">
          ...
          {sorted.length === 0 && (<tr><td colSpan={7 + customFields.length} className="py-12 text-center text-sm text-muted-foreground">No tasks</td></tr>)}
        </tbody>
      </table>
    </div>
  );
        ...
        {tasks.map((t) => {
          ...
          return (
            <div key={t.id} className="flex border-b border-border/60" style={{ height: rowH }}>
              <div className="flex w-60 shrink-0 items-center gap-2 truncate px-3 text-xs"><PriorityIcon priority={t.priority} /><span className="truncate">{t.title}</span></div>
              <div className="relative" style={{ width: colWidth }}>
                <button onClick={() => goTask(t)} className="absolute h-5 rounded bg-accent/70 px-2 text-[10px] text-accent-foreground hover:bg-accent" style={{ left: p.x, width: p.w, top: 8 }}>
                  {t.title.slice(0, 24)}
                </button>
              </div>
            </div>
